...
- **Implementation:** Etigel & Samuel (parallel tracks) · **Agile tracking & documentation:** Kei, Shane
...
|---|-------|:--------:|:-----------:|-------|
| 6 | [Register, log in & log out](./user_stories/user_story_05_register_login.md) | P1 | 1.5 | Etigel |
| 7 | [Checkout & pay online (Stripe)](./user_stories/user_story_06_checkout_payment.md) | P1 | 2.5 | Etigel |
| 8 | Order confirmation & my orders | P2 | 1.0 | Etigel |
| 9 | Admin: manage products & orders | P1 | 1.0 | Etigel |
| 10 | [Admin: total quantity ordered per product](./user_stories/user_story_07_admin_aggregate.md) *(USP)* | P1 | 1.5 | Etigel |
...
| 14 | Export shopping list (CSV) | P3 | 0.5 | Etigel |

**Total: 11.0 days.**

## In progress → Completed

- Checkout + Stripe Checkout (test) + confirmation — **completed 22 Jul (Etigel)**
- `Order`/`OrderItem` models, order history, status badges — **completed 22 Jul (Etigel)**
- Register/login (by email), auth-aware header — **completed 22–23 Jul (Etigel)**
- Admin aggregate quantities (USP) + CSV export + order-status actions — **completed 23 Jul (Etigel)**
- `CustomerProfile`: saved details, favourites, reorder + **10 automated tests** — **completed 22–23 Jul (Samuel)**
- Containerised deployment to Hostinger VPS (Docker/Gunicorn) — **completed 23 Jul (Etigel)**
- Merge of the two developers' branches (no conflicts) — **23 Jul**
- Production hardening: gunicorn workers, swap, Stripe key fix; end-to-end paid order **#PP-00016** — **completed 24 Jul (Etigel)**
